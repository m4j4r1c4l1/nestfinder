// Russian translations
export default {
    _meta: {
        code: 'ru',
        name: 'Russian',
        nativeName: 'Русский',
        flag: '🇷🇺'
    },
    // Feedback (English fallback)
    feedback: {
        description: 'Сообщайте об ошибках, предлагайте функции или отправляйте отзывы',
        placeholder: 'Опишите ваш отзыв подробно...',
        send: 'Отправить отзыв',
        charLimit: 'символов',
        rateApp: 'Оценить приложение',
        bugReport: 'Отчет об ошибке',
        suggestion: 'Предложение',
        general: 'Общее',
        sentStatus: '✓ Отправлено!',
        sending: 'Отправка...',
        types: {
            bug: '🐛 Ошибка',
            suggestion: '💡 Идея',
            other: '📝 Другое'
        },
        title: 'Feedback',
        description: 'Report bugs, suggest features, or send feedback',
        placeholder: 'Describe your feedback in detail...',
        send: 'Send Feedback',
        charLimit: 'characters',
        rateApp: 'Rate the App',
        bugReport: 'Bug Report',
        suggestion: 'Suggestion',
        general: 'Feedback'
    },
    nav: {
        map: 'Карта',
        report: 'Сообщить',
        profile: 'Профиль',
        inbox: 'Входящие',
        settings: 'Настройки'
    },
    map: {
        searchPlaceholder: 'Поиск места...',
        locateMe: 'Мое местоположение',
        filters: 'Фильтры',
        route: 'Маршрут',
        download: 'Скачать',
        downloadGPX: 'Скачать GPX',
        downloadKML: 'Скачать KML',
        downloadJSON: 'Скачать JSON',
        downloadCSV: 'Скачать CSV',
        totalPoints: 'всего точек',
        noResults: 'Результаты не найдены'
    },
    filters: {
        title: 'Фильтр точек',
        showConfirmed: 'Показать подтвержденные',
        showPending: 'Показать на проверке',
        showDeactivated: 'Показать неактивные',
        done: 'Готово',
        reset: 'Сброс'
    },
    status: {
        confirmed: 'Подтверждено',
        pending: 'На проверке',
        deactivated: 'Неактивно'
    },
    point: {
        details: 'Детали точки',
        status: 'Статус',
        address: 'Адрес',
        notes: 'Заметки',
        submittedBy: 'Отправлено',
        confirmations: 'Подтверждения',
        deactivations: 'Жалобы',
        actions: 'Действия',
        confirmBtn: 'Подтвердить активность',
        deactivateBtn: 'Сообщить о неактивности',
        reactivateBtn: 'Активировать снова',
        navigateBtn: 'Навигация',
        close: 'Закрыть',
        anonymous: 'Аноним',
        confirmedMessage: 'Спасибо за подтверждение!',
        deactivatedMessage: 'Сообщено о неактивности',
        reactivatedMessage: 'Точка снова активна!'
    },
    submit: {
        foundPrefix: 'Найдено:',
        locationFound: 'Местоположение найдено!',
        title: 'Сообщить о месте',
        subtitle: 'Помогите другим, сообщив о месте',
        addressLabel: 'Адрес',
        addressPlaceholder: 'Введите адрес или используйте карту',
        notesLabel: 'Заметки (необязательно)',
        notesPlaceholder: 'Дополнительные детали...',
        submitBtn: 'Отправить',
        submitting: 'Отправка...',
        success: 'Место успешно добавлено!',
        error: 'Ошибка при отправке. Попробуйте снова.',
        selectOnMap: 'Или выберите на карте',
        currentLocation: 'Использовать текущее местоположение',
        gpsMode: 'GPS',
        mapMode: 'Карта',
        addressMode: 'Адрес',
        currentLocationLabel: 'Текущее местоположение',
        tapToLocate: 'Нажмите для поиска',
        selectedLocation: 'Выбранное место',
        locationSelected: 'Место выбрано',
        tapMapPrompt: 'Нажмите на карту, чтобы выбрать место',
        mapInstructions: 'Закройте панель, нажмите на карту, и панель снова откроется с выбранным местом.',
        cityLabel: 'Город',
        cityPlaceholder: 'напр. Москва',
        streetLabel: 'Улица',
        streetPlaceholder: 'напр. Ленина',
        numberLabel: 'Номер',
        numberPlaceholder: 'напр. 42',
        findLocation: 'Найти место',
        addressNotFound: 'Адрес не найден.',
        geocodeError: 'Ошибка геокодирования.',
        addressRequired: 'Введите город и улицу',
        locationRequired: 'Требуется местоположение.',
        tagsLabel: 'Кто там? (Опционально)',
        onePerson: 'Один человек',
        multiple: 'Группа',
        children: 'Дети',
        animals: 'Животные'
        ,
        voiceMode: 'Голос',
        needsLabel: 'Что им нужно? (Опционально)',
        needFood: 'Еда',
        needWater: 'Вода',
        needClothes: 'Одежда',
        needMedicine: 'Лекарства',
        needShelter: 'Убежище'
    },
    route: {
        needPointsError: 'Need at least 2 points to calculate a route',
        failed: 'Could not calculate route',
        title: 'Планировщик маршрута',
        optimizeRoute: 'Рассчитать маршрут',
        clearRoute: 'Очистить маршрут',
        calculating: 'Вычисление...',
        distance: 'Расстояние',
        duration: 'Время',
        waypoints: 'точки',
        noPoints: 'Нет доступных точек для маршрута',
        filterByStatus: 'Фильтр по статусу',
        includeConfirmed: 'Вкл. подтвержденные',
        includePending: 'Вкл. на проверке',
        includeDeactivated: 'Вкл. неактивные',
        pointsSelected: 'Выбрано точек: {n}',
        readyDescription: 'Calculate optimized walking path visiting all selected points.'
    },
    inbox: {
        noFilterMatch: 'No messages match this filter',
        title: 'Сообщения',
        received: 'Входящие',
        sent: 'Отправленные',
        compose: 'Написать',
        noMessages: 'Сообщений пока нет',
        markAllRead: 'Отметить все как прочитанные',
        unread: 'непрочитанные',
        empty: 'Ваше гнездо пусто',
        noSent: 'Пока нет вкладов',
        'delete.confirm': 'Удалить это сообщение навсегда?',
        'delete.cancel': 'Отмена',
        'delete.yes': 'Удалить'
    },
    profile: {
        title: 'Профиль',
        nickname: 'Никнейм',
        nicknamePlaceholder: 'Ваш никнейм',
        language: 'Язык',
        pointsSubmitted: 'Точек отправлено',
        confirmationsMade: 'Подтверждений сделано',
        saveChanges: 'Сохранить',
        saving: 'Сохранение...',
        saved: 'Изменения сохранены!',
        deviceId: 'ID устройства',
        memberSince: 'Участник с',
        statistics: 'Статистика'
    },
    language: {
        title: 'Выбор языка',
        subtitle: 'Выберите предпочитаемый язык',
        continue: 'Продолжить'
    },
    settings: {
        deleteActions: 'Удалить сообщения',
        yourProfile: 'Ваш Профиль',
        notifications: 'Уведомления',
        popupMessages: 'Всплывающие сообщения',
        popupDescription: 'Показывать сообщения сразу',
        popupEnabledInfo: 'Сообщения будут всплывать при получении.',
        popupDisabledInfo: 'Сообщения не будут всплывать, но сохранятся во входящих.',
        shareApp: 'Поделиться NestFinder',
        scanToShare: 'Сканируйте для NestFinder',
        copyLink: 'Копировать ссылку',
        shareLink: 'Поделиться ссылкой',
        linkCopied: 'Ссылка скопирована!',
        recoveryKey: 'Ключ восстановления',
        recoveryKeyDescription: 'Сохраните этот ключ для восстановления вашего профиля на новом устройстве.',
        generateKey: 'Создать ключ восстановления',
        showKey: 'Показать ключ восстановления',
        copyKey: 'Копировать ключ',
        copied: 'Скопировано!',
        keyGenerated: 'Ключ создан и скопирован!',
        performance: 'Производительность',
        liteMode: 'Легкий режим',
        liteModeDescription: 'Уменьшить анимации для плавной работы',
        scrollInstruction: '🌍 Прокрутите + Нажмите или подождите 2с для подтверждения',
        spreadWarmth: 'Распространяйте тепло! 🐣',
        trustScore: 'Оценка доверия',
        anonymousUser: 'Пользователь',
        statusHatchling: 'Птенец',
        statusSparrow: 'Воробей',
        statusOwl: 'Сова',
        statusEagle: 'Орёл',
        // Recovery Key Restore
        // recoveryKeyUsage: Deprecated
        restoreOptionsTitle: 'Чтобы восстановить аккаунт, у вас есть 2 варианта:',
        restoreOption1: '**Вход:** Введите ваш ключ из 3 слов **через дефис** в поле **Никнейм** при входе в **Nestfinder**.',
        restoreOption2: '**Новый аккаунт:** Войдите с новым аккаунтом и восстановите старый в разделе **Восстановить аккаунт** ниже.',
        restoreAccount: 'Восстановить аккаунт',
        restoreAccountDescription: 'Введите ключ восстановления для восстановления вашей личности.',
        enterRecoveryKey: 'слово-слово-слово',
        invalidRecoveryKey: 'Неверный ключ восстановления. Проверьте и попробуйте снова.',
        accountRestored: 'Аккаунт восстановлен! Перезагрузка...',
        restoreButton: 'Восстановить аккаунт',
        // Restore Warning Dialog
        restoreWarningTitle: 'Предупреждение',
        restoreWarningMessage: 'Восстановление другого аккаунта отключит вас от текущего аккаунта. Если вы не сохранили ключ восстановления этого аккаунта, вы потеряете доступ навсегда. Продолжить?',
        restoreConfirmButton: 'Да, Восстановить',
        sameKeyError: 'Это ключ восстановления вашего текущего аккаунта. Вы уже вошли с этим аккаунтом.',
        // Swipe Direction
        swipeDirection: 'Удалить сообщения',
        deleteSettingDesc: 'Выберите способ удаления сообщения',
        safeDelete: 'Безопасное удаление',
        tapToSelect: 'Нажмите, чтобы выбрать направление смахивания',
        swipe: {
            controlLabel: 'Swipe',
            right: '→ Свайп вправо',
            left: '← Свайп влево',
            desc: 'Проведите по сообщению в этом направлении, чтобы удалить.'
        },
        // Message Retention
        messageRetention: 'Хранение сообщений',
        retention: {
            '1m': '1 Месяц',
            '3m': '3 Месяца',
            '6m': '6 Месяцев',
            forever: 'Навсегда',
            desc: '⚠️ Старые сообщения будут удалены навсегда.'
        },
        retentionHelp: {
            forever: 'Сообщения хранятся <b>бессрочно</b>.',
            read: 'Сообщения будут удалены после <b>прочтения</b>.',
            period: 'Сообщения старше <b>{time}</b> будут удалены.',
            unit: {
                d: 'День',
                w: 'Неделя',
                m: 'Месяц',
                y: 'Год',
                h: 'Час'
            }
        },
        swipeHelp: {
            left: 'Теперь вы можете свайпнуть <b>влево</b> по сообщению для удаления',
            right: 'Теперь вы можете свайпнуть <b>вправо</b> по сообщению для удаления',
            both: 'Теперь вы можете свайпнуть <b>влево</b> или <b>вправо</b> для удаления'
        },
        safeDeleteHelp: {
            enabled: 'Кнопка <delete>Удалить</delete> появится при свайпе сообщения.',
            disabled: 'Теперь вы можете удалить сообщение просто смахнув его.'
        }
    },
    // Welcome Message (Home Page)
    // Welcome Message (Home Page & Modal)
    welcome: {
        // Home Screen
        title: 'NestFinder',
        subtitle: 'Поиск Убежищ ❤️ Принося Облегчение',
        nicknameLabel: 'Введите псевдоним для участия (необязательно)',
        nicknamePlaceholder: 'Анонимный Помощник',
        buttonStart: 'Начать Помогать',
        buttonLoading: 'Запуск...',

        // Welcome Modal
        modalTitle: 'Добро пожаловать в NestFinder!',
        message1: 'Спасибо за то, что вы замечательный человек!',
        message2: 'Каждый добрый поступок важен.',
        message3: 'Вместе мы можем изменить мир к лучшему.',
        callToAction: 'Помогайте находить и поддерживать тех, кто в этом нуждается.',
        button: 'Начать',
        // Recovery error
        invalidRecoveryKey: 'Неверный ключ восстановления. Проверьте и попробуйте снова.'
    },
    common: {
        loading: 'Загрузка...',
        error: 'Произошла ошибка',
        retry: 'Повторить',
        cancel: 'Отмена',
        save: 'Сохранить',
        delete: 'Удалить',
        confirm: 'Подтвердить',
        close: 'Закрыть',
        back: 'Назад',
        next: 'Далее',
        yes: 'Да',
        no: 'Нет',
        ok: 'ОК'
    },
    geo: {
        permissionDenied: 'Доступ к геолокации запрещен',
        unavailable: 'Геолокация недоступна',
        timeout: 'Время ожидания истекло',
        enableLocation: 'Включить геолокацию',
        requestingLocation: 'Поиск местоположения...',
        enableTitle: 'Включите геолокацию',
        enableSubtitle: 'Нажмите ниже для включения местоположения и персонализированных маршрутов',
        enableButton: '📍 Включить геолокацию',
        locationEnabled: 'Геолокация включена!',
        locationBlocked: 'Геолокация заблокирована. Очистите данные браузера и попробуйте снова.',
        locationDenied: 'Доступ запрещен. Проверьте {tip}',
        locationUnavailable: 'Геолокация недоступна. Проверьте GPS.',
        locationTimeout: 'Время ожидания истекло. Попробуйте снова или проверьте GPS.',
        iosInstructions: 'iOS: Настройки → Конфиденциальность → Службы геолокации → Safari → Разрешить',
        androidInstructions: 'Android: Настройки → Приложения → Браузер → Разрешения → Местоположение → Разрешить',
        desktopInstructions: 'Проверьте настройки браузера для включения геолокации',
        iosTip: 'Настройки → Конфиденциальность → Службы геолокации → Safari',
        androidTip: 'Настройки → Приложения → Браузер → Разрешения → Местоположение',
        browserSettings: 'настройки браузера'
    },
    validation: {
        required: 'Это поле обязательно',
        invalidAddress: 'Введите корректный адрес',
        tooShort: 'Слишком коротко',
        tooLong: 'Слишком длинно'
    },
    time: {
        justNow: 'Только что',
        minutesAgo: '{n} мин. назад',
        hoursAgo: '{n} ч. назад',
        daysAgo: '{n} дн. назад',
        today: 'Сегодня',
        yesterday: 'Вчера'
    }
};
